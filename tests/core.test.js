...
// Native percentage tests (Codex CLI v2.1.6+)
...
test('parseTranscript prefers custom title over slug for session name', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'codex-hud-'));
...
test('countConfigs honors project and global config locations', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-project-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;

  try {
    await mkdir(path.join(homeDir, '.codex', 'rules', 'nested'), { recursive: true });
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.md'), 'global', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'rules', 'rule.md'), '# rule', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'rules', 'nested', 'rule-nested.md'), '# rule nested', 'utf8');
    await writeFile(
      path.join(homeDir, '.codex', 'settings.json'),
      JSON.stringify({ mcpServers: { one: {} }, hooks: { onStart: {} } }),
      'utf8'
    );
    await writeFile(path.join(homeDir, '.codex.json'), '{bad json', 'utf8');

    await mkdir(path.join(projectDir, '.codex', 'rules'), { recursive: true });
    await writeFile(path.join(projectDir, 'AGENTS.md'), 'project', 'utf8');
    await writeFile(path.join(projectDir, 'AGENTS.local.md'), 'project-local', 'utf8');
    await writeFile(path.join(projectDir, '.codex', 'AGENTS.md'), 'project-alt', 'utf8');
    await writeFile(path.join(projectDir, '.codex', 'AGENTS.local.md'), 'project-alt-local', 'utf8');
    await writeFile(path.join(projectDir, '.codex', 'rules', 'rule2.md'), '# rule2', 'utf8');
    await writeFile(
      path.join(projectDir, '.codex', 'settings.json'),
      JSON.stringify({ mcpServers: { two: {}, three: {} }, hooks: { onStop: {} } }),
      'utf8'
    );
    await writeFile(path.join(projectDir, '.codex', 'settings.local.json'), '{bad json', 'utf8');
    ...
    assert.equal(counts.agentsMdCount, 5);
...
test('countConfigs uses CODEX_HOME and matching .json sidecar for user scope', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const customConfigDir = path.join(homeDir, '.codex-2');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HOME;
  process.env.HOME = homeDir;
  process.env.CODEX_HOME = customConfigDir;

  try {
    // Default directory should be ignored when CODEX_HOME is set.
    await mkdir(path.join(homeDir, '.codex', 'rules'), { recursive: true });
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.md'), 'default-global', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'rules', 'rule.md'), '# default rule', 'utf8');
    await writeFile(
      path.join(homeDir, '.codex', 'settings.json'),
      JSON.stringify({ mcpServers: { defaultA: {} }, hooks: { onDefault: {} } }),
      'utf8'
    );
    await writeFile(path.join(homeDir, '.codex.json'), JSON.stringify({ disabledMcpServers: ['defaultA'] }), 'utf8');

    // Custom config directory and sidecar should drive user-scope counts.
    await mkdir(customConfigDir, { recursive: true });
    await writeFile(path.join(customConfigDir, 'AGENTS.md'), 'custom-global', 'utf8');
    await writeFile(
    ...
    assert.equal(counts.agentsMdCount, 1);
    assert.equal(counts.rulesCount, 0);
    assert.equal(counts.mcpCount, 1);
    assert.equal(counts.hooksCount, 2);
  } finally {
    restoreEnvVar('HOME', originalHome);
    restoreEnvVar('CODEX_HOME', originalConfigDir);
    await rm(homeDir, { recursive: true, force: true });
  }
});

test('countConfigs still counts project .codex when cwd is home and CODEX_HOME points elsewhere', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const customConfigDir = path.join(homeDir, '.codex-2');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HOME;
  ...
  try {
    ...
    // Project scope: cwd is home directory with its own .codex contents
    await mkdir(path.join(homeDir, '.codex', 'rules'), { recursive: true });
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.md'), 'project-alt', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'rules', 'project-rule.md'), '# project rule', 'utf8');
    await writeFile(
      path.join(homeDir, '.codex', 'settings.json'),
    ...
    assert.equal(counts.agentsMdCount, 2);
...
test('countConfigs avoids home cwd double-counting across counters and keeps AGENTS.local.md', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
  ...
  try {
    await mkdir(path.join(homeDir, '.codex', 'rules'), { recursive: true });
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.md'), 'global', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.local.md'), 'global-local', 'utf8');
    await writeFile(path.join(homeDir, '.codex', 'rules', 'rule.md'), '# rule', 'utf8');
    await writeFile(
      path.join(homeDir, '.codex', 'settings.json'),
      JSON.stringify({ mcpServers: { one: {} }, hooks: { onStart: {} } }),
    ...
    assert.equal(exactCounts.agentsMdCount, 2);
    ...
    assert.equal(trailingSlashCounts.agentsMdCount, 2);
...
test('countConfigs excludes disabled user-scope MCPs', async () => {
  ...
  try {
    ...
    // 1 MCP disabled in ~/.codex.json
    await writeFile(
      path.join(homeDir, '.codex.json'),
...
test('countConfigs tolerates rule directory read errors', async () => {
  ...
  const rulesDir = path.join(homeDir, '.codex', 'rules');
...
// https://github.com/jarrodwatts/codex-hud/issues/3
