...
test('getConfigPath returns correct path', () => {
  ...
  try {
    ...
    assert.equal(configPath, path.join(homeDir, '.codex', 'plugins', 'codex-hud', 'config.json'));
  } finally {
...
test('getConfigPath respects CODEX_HOME', async () => {
  const originalConfigDir = process.env.CODEX_HOME;
  const customConfigDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-config-dir-'));

  try {
    process.env.CODEX_HOME = customConfigDir;
    const configPath = getConfigPath();
    assert.equal(configPath, path.join(customConfigDir, 'plugins', 'codex-hud', 'config.json'));
  } finally {
    restoreEnvVar('CODEX_HOME', originalConfigDir);
...
test('loadConfig reads user config from CODEX_HOME', async () => {
  const originalConfigDir = process.env.CODEX_HOME;
  const customConfigDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-config-load-'));

  try {
    process.env.CODEX_HOME = customConfigDir;
    const pluginDir = path.join(customConfigDir, 'plugins', 'codex-hud');
