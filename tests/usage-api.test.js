...
async function createTempHome() {
  return await mkdtemp(path.join(tmpdir(), 'codex-hud-usage-'));
}
...
async function writeCredentials(homeDir, credentials) {
  await writeCredentialsInConfigDir(path.join(homeDir, '.codex'), credentials);
}
...
describe('resolveKeychainCredentials', () => {
  test('falls back to legacy service when profile-specific service is missing', () => {
    const now = 1000;
    const serviceNames = ['Codex CLI-credentials-deadbeef', 'Codex CLI-credentials'];
    const calls = [];

    const result = resolveKeychainCredentials(serviceNames, now, (serviceName) => {
      calls.push(serviceName);
      if (serviceName === 'Codex CLI-credentials-deadbeef') {
  ...
  test('treats missing-item message as non-backoff condition', () => {
    const now = 1000;
    const serviceNames = ['Codex CLI-credentials-hashed'];
  ...
  test('uses first valid credential in candidate order', () => {
    const now = 1000;
    const serviceNames = ['Codex CLI-credentials-canonical', 'Codex CLI-credentials-fallback'];

    const result = resolveKeychainCredentials(serviceNames, now, (serviceName) => {
      if (serviceName === 'Codex CLI-credentials-canonical') {
        return JSON.stringify(buildCredentials({
  ...
  test('prefers account-scoped keychain entries before generic fallback', () => {
    const now = 1000;
    const serviceNames = ['Codex CLI-credentials'];
    const calls = [];
    ...
    assert.equal(result.shouldBackoff, false);
    assert.deepEqual(calls, [{ serviceName: 'Codex CLI-credentials', accountName: 'jarrod' }]);
  });
  ...
  test('falls back to generic lookup when account-scoped entry is missing', () => {
    ...
    assert.deepEqual(calls, [
      { serviceName: 'Codex CLI-credentials', accountName: 'jarrod' },
      { serviceName: 'Codex CLI-credentials', accountName: null },
    ]);
  });
  ...
  test('does not fall back to generic credentials from another service when a later account-scoped entry is unusable', () => {
    const now = 1000;
    const serviceNames = ['Codex CLI-credentials-hashed', 'Codex CLI-credentials'];
    const calls = [];
    ...
    const result = resolveKeychainCredentials(
      ...
      (serviceName, accountName) => {
        calls.push({ serviceName, accountName: accountName ?? null });
        if (accountName === 'jarrod' && serviceName === 'Codex CLI-credentials-hashed') {
          throw buildMissingKeychainError();
        }

        if (accountName === 'jarrod' && serviceName === 'Codex CLI-credentials') {
          return JSON.stringify({ mcpOAuth: { accessToken: 'wrong-scope' } });
        }
    ...
    assert.equal(result.shouldBackoff, false);
    assert.deepEqual(calls, [
      { serviceName: 'Codex CLI-credentials-hashed', accountName: 'jarrod' },
      { serviceName: 'Codex CLI-credentials', accountName: 'jarrod' },
    ]);
  });
});
...
describe('getUsage', () => {
  ...
  test('reads credentials from CODEX_HOME and prefers them over default path', async () => {
    const originalConfigDir = process.env.CODEX_HOME;
    const customConfigDir = path.join(tempHome, '.codex-2');
  ...
  test('writes usage cache under CODEX_HOME plugin directory', async () => {
    const originalConfigDir = process.env.CODEX_HOME;
    const customConfigDir = path.join(tempHome, '.codex-2');
    process.env.CODEX_HOME = customConfigDir;
    ...
    try {
      ...
      const customCachePath = path.join(customConfigDir, 'plugins', 'codex-hud', '.usage-cache.json');
      const defaultCachePath = path.join(tempHome, '.codex', 'plugins', 'codex-hud', '.usage-cache.json');
...
test('usage API user agent uses a non-empty codex-hud identifier', () => {
...
describe('getKeychainServiceName', () => {
  test('uses legacy default service name for default config directory', () => {
    const homeDir = '/tmp/codex-hud-home-default';
    const defaultConfigDir = path.join(homeDir, '.codex');
    const serviceName = getKeychainServiceName(defaultConfigDir, homeDir);
    assert.equal(serviceName, 'Codex CLI-credentials');
  });

  test('uses profile-specific hashed service name for custom config directory', () => {
    const homeDir = '/tmp/codex-hud-home-custom';
    ...
    assert.equal(serviceName, `Codex CLI-credentials-${expectedHash}`);
  });

  test('treats normalized default path as legacy service name', () => {
    const homeDir = '/tmp/codex-hud-home-normalized';
    const serviceName = getKeychainServiceName(path.join(homeDir, '.codex', '..', '.codex'), homeDir);
    assert.equal(serviceName, 'Codex CLI-credentials');
  });
});
...
describe('getKeychainServiceNames', () => {
  test('includes both env-hash and normalized-dir hash candidates before legacy fallback', () => {
    const homeDir = '/tmp/codex-hud-home-candidates';
    const configDir = path.join(homeDir, '.codex-2');
    const envConfigDir = '~/.codex-2';
    ...
    const serviceNames = getKeychainServiceNames(configDir, homeDir, { CODEX_HOME: envConfigDir });

    assert.deepEqual(serviceNames, [
      `Codex CLI-credentials-${normalizedHash}`,
      `Codex CLI-credentials-${envHash}`,
      'Codex CLI-credentials',
    ]);
  });

  test('returns legacy-only when config resolves to default location', () => {
    const homeDir = '/tmp/codex-hud-home-default-candidates';
    const defaultConfigDir = path.join(homeDir, '.codex');

    const serviceNames = getKeychainServiceNames(defaultConfigDir, homeDir, {});

    assert.deepEqual(serviceNames, ['Codex CLI-credentials']);
  });

  test('returns legacy-only when env also points to default location', () => {
    const homeDir = '/tmp/codex-hud-home-default-env';
    const defaultConfigDir = path.join(homeDir, '.codex');

    const serviceNames = getKeychainServiceNames(
      defaultConfigDir,
      homeDir,
      { CODEX_HOME: defaultConfigDir }
    );
...
describe('getUsage caching behavior', { concurrency: false }, () => {
  ...
  test('treats zero-byte lock file as stale and fetches fresh data', async () => {
    ...
    try {
      ...
      const pluginDir = path.join(cacheTempHome, '.codex', 'plugins', 'codex-hud');
