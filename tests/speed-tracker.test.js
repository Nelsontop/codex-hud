...
async function createTempHome() {
  return await mkdtemp(path.join(tmpdir(), 'codex-hud-speed-'));
}
...
test('getOutputSpeed writes cache under CODEX_HOME by default', async () => {
  const tempHome = await createTempHome();
  const customConfigDir = path.join(tempHome, '.codex-alt');
  ...
  try {
    ...
    const customCachePath = path.join(customConfigDir, 'plugins', 'codex-hud', '.speed-cache.json');
    const defaultCachePath = path.join(tempHome, '.codex', 'plugins', 'codex-hud', '.speed-cache.json');
