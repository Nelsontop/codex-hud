...
test('main logs an error when dependencies throw', async () => {
  ...
  await main({
    ...
    countConfigs: async () => ({ agentsMdCount: 0, rulesCount: 0, mcpCount: 0, hooksCount: 0 }),
  ...
  assert.ok(logs.some((line) => line.includes('[codex-hud] Error:')));
});
...
test('index entrypoint runs when executed directly', async () => {
  ...
  assert.ok(logs.some((line) => line.includes('[codex-hud] Initializing...')));
});
