---
description: Configure codex-hud as your statusline
...
CACHE_EXISTS=$(ls -d ~/.codex/plugins/cache/codex-hud 2>/dev/null && echo "YES" || echo "NO")

# Check 2: Registry entry exists?
REGISTRY_EXISTS=$(grep -q "codex-hud" ~/.codex/plugins/installed_plugins.json 2>/dev/null && echo "YES" || echo "NO")

# Check 3: Temp files left behind?
TEMP_FILES=$(ls -d ~/.codex/plugins/cache/temp_local_* 2>/dev/null | head -1)
...
**Windows (PowerShell)**:
```powershell
$cache = Test-Path "$env:USERPROFILE\.codex\plugins\cache\codex-hud"
$registry = (Get-Content "$env:USERPROFILE\.codex\plugins\installed_plugins.json" -ErrorAction SilentlyContinue) -match "codex-hud"
$temp = Get-ChildItem "$env:USERPROFILE\.codex\plugins\cache\temp_local_*" -ErrorAction SilentlyContinue
...
rm -rf ~/.codex/plugins/cache/codex-hud

# Remove temp files from failed installs
rm -rf ~/.codex/plugins/cache/temp_local_*

# Reset registry (removes ALL plugins - warn user first!)
# Only run if user confirms they have no other plugins they want to keep:
echo '{"version": 2, "plugins": {}}' > ~/.codex/plugins/installed_plugins.json
```

**Windows (PowerShell)**:
```powershell
# Remove orphaned cache
Remove-Item -Recurse -Force "$env:USERPROFILE\.codex\plugins\cache\codex-hud" -ErrorAction SilentlyContinue

# Remove temp files
Remove-Item -Recurse -Force "$env:USERPROFILE\.codex\plugins\cache\temp_local_*" -ErrorAction SilentlyContinue

# Reset registry (removes ALL plugins - warn user first!)
'{"version": 2, "plugins": {}}' | Set-Content "$env:USERPROFILE\.codex\plugins\installed_plugins.json"
```

After cleanup, tell user to **restart Codex CLI** and run `/plugin install codex-hud` again.
...
```bash
mkdir -p ~/.cache/tmp && TMPDIR=~/.cache/tmp codex
```

This is a [Codex CLI platform limitation](https://github.com/anthropics/claude-code/issues/14799).
...
1. Get plugin path (sorted by dotted numeric version, not modification time):
   ```bash
   ls -d "$HOME"/.codex/plugins/cache/codex-hud/codex-hud/*/ 2>/dev/null | awk -F/ '{ print $(NF-1) "\t" $0 }' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n | tail -1 | cut -f2-
   ```
   If empty, the plugin is not installed. Go back to Step 0 to check for ghost installation or EXDEV issues. If Step 0 was clean, tell user to install via `/plugin install codex-hud` first.
...
5. Generate command (quotes around runtime path handle spaces):
   ```
   bash -c 'plugin_dir=$(ls -d "$HOME"/.codex/plugins/cache/codex-hud/codex-hud/*/ 2>/dev/null | awk -F/ '"'"'{ print $(NF-1) "\t" $0 }'"'"' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n | tail -1 | cut -f2-); exec "{RUNTIME_PATH}" "${plugin_dir}{SOURCE}"'
   ```
...
- Any other shell value -> stop and ask the user which shell launched Codex CLI.

1. Get plugin path:
   ```powershell
   (Get-ChildItem "$env:USERPROFILE\.codex\plugins\cache\codex-hud\codex-hud" -Directory | Where-Object { $_.Name -match '^\d+(\.\d+)+$' } | Sort-Object { [version]$_.Name } -Descending | Select-Object -First 1).FullName
   ```
...
4. Generate command (note: quotes around runtime path handle spaces in paths):
   ```
   powershell -Command "& {$p=(Get-ChildItem $env:USERPROFILE\.codex\plugins\cache\codex-hud\codex-hud -Directory | Where-Object { $_.Name -match '^\d+(\.\d+)+$' } | Sort-Object { [version]$_.Name } -Descending | Select-Object -First 1).FullName; & '{RUNTIME_PATH}' (Join-Path $p '{SOURCE}')}"
   ```

**WSL (Windows Subsystem for Linux)**: If running in WSL, use the macOS/Linux instructions. Ensure the plugin is installed in the Linux environment (`~/.codex/plugins/...`), not the Windows side.
...
- **Platform `darwin` or `linux`, or Platform `win32` + Shell `bash`**: `~/.codex/settings.json`
- **Platform `win32` + Shell `powershell`, `pwsh`, or `cmd`**: `$env:USERPROFILE\.codex\settings.json`
...
**Note**: The generated command dynamically finds and runs the latest installed plugin version. Updates are automatic - no need to re-run setup after plugin updates. If the HUD suddenly stops working, re-run `/codex-hud:setup` to verify the plugin is still installed.
...
- options:
  ...
  - "Session info" — Shows session duration and config counts (AGENTS.md, rules, MCPs)
  - "Session name" — Shows session slug or custom title from /rename

**If user selects any options**, write `~/.codex/plugins/codex-hud/config.json` (create directories if needed):
...
**If yes**: Ask the user if they'd like to ⭐ star the codex-hud repository on GitHub to support the project. If they agree and `gh` CLI is available, run: `gh api -X PUT /user/starred/jarrodwatts/codex-hud`. Only run the star command if they explicitly say yes.

**If no**: Debug systematically:

1. **Verify config was applied**:
   - Read settings file (`~/.codex/settings.json` or `$env:USERPROFILE\.codex\settings.json` on Windows)
...
3. **Common issues to check**:
   ...
   **"No such file or directory" for plugin**:
   - Plugin might not be installed: `ls ~/.codex/plugins/cache/codex-hud/`
   ...
   **WSL confusion**:
   ...
   - Check: `ls ~/.codex/plugins/cache/codex-hud/`
