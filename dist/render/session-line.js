...
export function renderSessionLine(ctx) {
    ...
    if (DEBUG && autocompactMode === 'disabled') {
        console.error(`[codex-hud:context] autocompactBuffer=disabled, showing raw ${rawPercent}% (buffered would be ${bufferedPercent}%)`);
    }
    ...
    if (display?.showConfigCounts !== false) {
        ...
        if (totalCounts > 0 && totalCounts >= envThreshold) {
            if (ctx.agentsMdCount > 0) {
                parts.push(dim(`${ctx.agentsMdCount} AGENTS.md`));
            }
            if (ctx.rulesCount > 0) {
