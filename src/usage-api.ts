...
import { getCodexHomeDir, getHudPluginDir } from './codex-home-dir.js';

export type { UsageData } from './types.js';

const debug = createDebug('usage');
const LEGACY_KEYCHAIN_SERVICE_NAME = 'Codex CLI-credentials';
...
function getOauthCredentials(data: CredentialsFile): CredentialsFile['codexAiOauth'] | undefined {
  return data.codexAiOauth ?? data.claudeAiOauth;
}
...
/**
 * Determine the macOS Keychain service name for Codex CLI credentials.
 * Codex CLI uses the default service for ~/.codex and a hashed suffix for custom config directories.
 */
export function getKeychainServiceName(configDir: string, homeDir: string): string {
  ...
  const normalizedDefaultDir = path.normalize(path.resolve(path.join(homeDir, '.codex')));
...
export function getKeychainServiceNames(
  ...
  const envConfigDir = env.CODEX_HOME?.trim();
 ...
 * Codex CLI stores OAuth credentials in the macOS Keychain with profile-specific service names.
...
function readKeychainCredentials(now: number, homeDir: string): { accessToken: string; subscriptionType: string } | null {
  ...
  try {
    const configDir = getCodexHomeDir(homeDir);
 ...
 * Older versions of Codex CLI stored credentials in {CODEX_HOME}/.credentials.json.
 */
function readFileCredentials(homeDir: string, now: number): { accessToken: string; subscriptionType: string } | null {
  const credentialsPath = path.join(getCodexHomeDir(homeDir), '.credentials.json');
...
function readFileSubscriptionType(homeDir: string): string | null {
  ...
  try {
    const content = fs.readFileSync(credentialsPath, 'utf8');
    const data: CredentialsFile = JSON.parse(content);
    const subscriptionType = getOauthCredentials(data)?.subscriptionType;
...
function parseCredentialsData(data: CredentialsFile, now: number): { accessToken: string; subscriptionType: string } | null {
  const oauth = getOauthCredentials(data);
...
/**
 * Read OAuth credentials, trying macOS Keychain first (Codex CLI 2.x),
 * then falling back to file-based credentials (older versions).
 *
 * Token priority: Keychain token is authoritative (Codex CLI 2.x stores current token there).
...
function readCredentials(
  ...
  // Try macOS Keychain first (Codex CLI 2.x)
